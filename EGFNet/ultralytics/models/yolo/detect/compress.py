...
from ultralytics.nn.extra_modules.block import Faster_Block, Fusion, IFM, InjectionMultiSum_Auto_pool, TopBasicLayer, SimFusion_3in, SimFusion_4in, AdvPoolFusion, \
    PyramidPoolAgg, RepVGGBlock, RepConvN, Star_Block, Faster_Block_CGLU, MutilScaleEdgeInformationEnhance, CSPOmniKernel, SPDConv,FeaturePyramidSharedConv
...
from ultralytics.nn.extra_modules.prune_module import C3k2_ConvFormerCGLU_v2, C3k2_CaFormerCGLU_v2
from ultralytics.nn.extra_modules.block import FeaturePyramidSharedConv
...
def get_pruner(opt, model, example_inputs):
    ...
    for k, m in model.named_modules():
        ...
        if isinstance(m, FeaturePyramidSharedConv):
            ignored_layers.append(m)
        if isinstance(m, C3k2_CaFormerCGLU_v2):
            ignored_layers.append(m)
        if isinstance(m, C3k2_ConvFormerCGLU_v2):
            ignored_layers.append(m)
