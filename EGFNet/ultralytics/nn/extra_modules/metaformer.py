...
class MF_Attention(nn.Module):
    ...
    def __init__(self, dim, head_dim=32, num_heads=None, qkv_bias=False,
        ...
        self.qkv = nn.Linear(dim, self.attention_dim * 3, bias=qkv_bias)#self.attention_dim * 3 是因为我们要同时生成 Query、Key 和 Value，每个部分的维度为 self.attention_dim。
    ...
    def forward(self, x):
        ...
        q, k, v = qkv.unbind(0)   # make torchscript happy (cannot use tensor as tuple) 此时，qkv 的形状是 (3, B, self.num_heads, N, self.head_dim)
#拆分三个张量
        ...
        x = self.proj(x)#改为BHWdim
        x = self.proj_drop(x)#全连接层之间放避免过拟合
        return x
...
class MetaFormerCGLUBlock(nn.Module):
    ...
    """
    def __init__(self, dim,
                 token_mixer=nn.Identity, mlp=ConvolutionalGLU,#此处改为CGLU
                 norm_layer=partial(LayerNormWithoutBias, eps=1e-6),
                 drop=0., drop_path=0.,
                 layer_scale_init_value=None, res_scale_init_value=None
                 ):
